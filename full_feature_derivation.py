"""
Phase 3: Full Feature Derivation for ALL ESPN Features

Derives all 319 ESPN features from nfl-data-py for 2024 season:
- 184 EXACT MATCH features
- 134 PARTIAL MATCH features
- 1 CLOSE APPROXIMATION feature (QB Rating)

Validates against ESPN ground truth and generates comprehensive reports.
"""
...
print("=" * 120)
print("PHASE 3: FULL FEATURE DERIVATION AND VALIDATION")
print("=" * 120)
...
print(f"  ✅ ESPN data: {len(espn)} teams, {len(espn.columns)} features")

# Load nfl-data-py
pbp = pd.read_parquet('data/cache/pbp_2024.parquet')
schedules = pd.read_parquet('data/cache/schedules_2024.parquet')
pbp_reg = pbp[pbp['week'] <= 18].copy()  # Regular season only
schedules_reg = schedules[schedules['week'] <= 18].copy()
print(f"  ✅ Play-by-play: {len(pbp_reg):,} plays (regular season)")
print(f"  ✅ Schedules: {len(schedules_reg)} games (regular season)")

# Load feature mapping
mapping = pd.read_csv('results/comprehensive_feature_mapping.csv')
features_to_derive = mapping[mapping['category'].isin(['EXACT MATCH', 'PARTIAL MATCH', 'CLOSE APPROXIMATION'])].copy()
print(f"  ✅ Features to derive: {len(features_to_derive)}")
print(f"     - EXACT MATCH: {len(features_to_derive[features_to_derive['category']=='EXACT MATCH'])}")
print(f"     - PARTIAL MATCH: {len(features_to_derive[features_to_derive['category']=='PARTIAL MATCH'])}")
print(f"     - CLOSE APPROXIMATION: {len(features_to_derive[features_to_derive['category']=='CLOSE APPROXIMATION'])}")

# ============================================================================
# STEP 2: FEATURE DERIVATION ENGINE
# ============================================================================

print("\n[2/6] Building feature derivation engine...")

def derive_all_features(team: str, pbp_reg: pd.DataFrame, schedules_reg: pd.DataFrame) -> dict:
    """
    Derive ALL 319 ESPN features for a team from nfl-data-py
    
    Args:
        team: ESPN team abbreviation (e.g., 'LAR', 'WSH')
        pbp_reg: Play-by-play data (regular season only, uses nfl-data-py abbreviations)
        schedules_reg: Schedule data (regular season only, uses nfl-data-py abbreviations)
...
print(f"     - Function: derive_all_features()")
print(f"     - Handles team abbreviation mapping (LAR↔LA, WSH↔WAS)")
print(f"     - Filters to regular season only (week <= 18)")
print(f"     - Derives ~319 features per team")

# ============================================================================
# STEP 3: DERIVE FEATURES FOR ALL TEAMS
# ============================================================================

print("\n[3/6] Deriving features for all 32 teams...")
...
for i, team in enumerate(teams, 1):
    print(f"  [{i:2d}/32] {team}...", end=' ', flush=True)
    try:
        team_features = derive_all_features(team, pbp_reg, schedules_reg)
        derived_data.append(team_features)
        print("✅")
    except Exception as e:
        print(f"❌ ERROR: {e}")
        derived_data.append({'team': team})  # Add empty row to maintain alignment

# Create derived features dataframe
derived = pd.DataFrame(derived_data).set_index('team').sort_index()
print(f"\n  ✅ Derived features: {len(derived)} teams, {len(derived.columns)} features")

# Save derived features
output_dir = Path('data/derived_features')
output_dir.mkdir(parents=True, exist_ok=True)
derived.to_parquet(output_dir / 'espn_derived_2024.parquet')
print(f"  ✅ Saved to: {output_dir / 'espn_derived_2024.parquet'}")

# ============================================================================
# STEP 4: VALIDATE AGAINST ESPN GROUND TRUTH
# ============================================================================

print("\n[4/6] Validating against ESPN ground truth...")

# Get common teams
common_teams = sorted(set(espn.index) & set(derived.index))
print(f"  ✅ Common teams: {len(common_teams)}")

# Get features to validate (exclude CANNOT DERIVE)
features_list = features_to_derive['espn_feature'].tolist()
available_features = [f for f in features_list if f in espn.columns and f in derived.columns]
...
for feature in available_features:
    try:
        ...
        if len(espn_clean) < 3:  # Need at least 3 points for correlation
            continue

        # Check for zero variance
        if np.std(espn_clean) == 0 or np.std(derived_clean) == 0:
            continue
        ...
        # Get category
        category = features_to_derive[features_to_derive['espn_feature'] == feature]['category'].iloc[0]
        ...
        print(f"  ⚠️  Error validating {feature}: {e}")

# Create validation results dataframe
validation_df = pd.DataFrame(validation_results)
validation_df = validation_df.sort_values('r', ascending=False)

print(f"  ✅ Validated {len(validation_df)} features")

# Save validation results
results_dir = Path('results')
validation_df.to_csv(results_dir / 'full_validation_results_2024.csv', index=False)
print(f"  ✅ Saved to: {results_dir / 'full_validation_results_2024.csv'}")
...
medium_count = len(validation_df[validation_df['status'] == 'MEDIUM'])
fail_count = len(validation_df[validation_df['status'] == 'FAIL'])

print(f"\n  Total features validated: {total_validated}")
print(f"    ✅ PASS: {pass_count} ({pass_count/total_validated*100:.1f}%)")
print(f"    ⚠️  MEDIUM: {medium_count} ({medium_count/total_validated*100:.1f}%)")
print(f"    ❌ FAIL: {fail_count} ({fail_count/total_validated*100:.1f}%)")

# Statistics by category
summary_stats = []

for category in ['EXACT MATCH', 'PARTIAL MATCH', 'CLOSE APPROXIMATION']:
    cat_df = validation_df[validation_df['category'] == category]

    if len(cat_df) == 0:
        continue

    cat_pass = len(cat_df[cat_df['status'] == 'PASS'])
    cat_medium = len(cat_df[cat_df['status'] == 'MEDIUM'])
    cat_fail = len(cat_df[cat_df['status'] == 'FAIL'])
    ...
    summary_stats.append({
        ...
        'medium': cat_medium,
        'fail': cat_fail,
        'pass_rate': cat_pass / len(cat_df) * 100,
        'mean_r': cat_df['r'].mean(),
        'median_r': cat_df['r'].median(),
        'mean_mape': cat_df['mape'].mean(),
        'median_mape': cat_df['mape'].median()
    })

    print(f"\n  {category}:")
    print(f"    Total: {len(cat_df)}")
    print(f"    Pass rate: {cat_pass/len(cat_df)*100:.1f}%")
    print(f"    Mean r: {cat_df['r'].mean():.4f}")
    print(f"    Median r: {cat_df['r'].median():.4f}")
    print(f"    Mean MAPE: {cat_df['mape'].mean():.1f}%")

# Save summary statistics
summary_df = pd.DataFrame(summary_stats)
summary_df.to_csv(results_dir / 'full_validation_summary_2024.csv', index=False)
print(f"\n  ✅ Saved summary to: {results_dir / 'full_validation_summary_2024.csv'}")

# ============================================================================
# STEP 6: DISPLAY TOP/BOTTOM FEATURES
# ============================================================================

print("\n[6/6] Top and bottom features...")

print("\n  🏆 TOP 20 BEST CORRELATIONS:")
top_20 = validation_df.head(20)
for idx, row in top_20.iterrows():
    print(f"    {row['feature']:50s} r={row['r']:.4f} (MAPE={row['mape']:5.1f}%) [{row['category']}]")

print("\n  💀 BOTTOM 20 WORST CORRELATIONS:")
bottom_20 = validation_df.tail(20)
...
print("\n" + "=" * 120)
print("✅ PHASE 3: FULL FEATURE DERIVATION COMPLETE!")
print("=" * 120)

print(f"\n📊 SUMMARY:")
print(f"  - Derived {len(derived.columns)} features for {len(derived)} teams")
print(f"  - Validated {total_validated} features against ESPN ground truth")
print(f"  - Overall pass rate: {pass_count/total_validated*100:.1f}%")
print(f"  - Mean correlation: r={validation_df['r'].mean():.4f}")
print(f"  - Median correlation: r={validation_df['r'].median():.4f}")

print(f"\n📁 OUTPUT FILES:")
print(f"  - Derived features: data/derived_features/espn_derived_2024.parquet")
print(f"  - Validation results: results/full_validation_results_2024.csv")
print(f"  - Summary statistics: results/full_validation_summary_2024.csv")

print(f"\n🎯 NEXT STEPS:")
print(f"  1. Review validation results to identify features to keep/exclude")
print(f"  2. Determine evidence-based correlation threshold")
print(f"  3. Create PHASE3_FULL_DERIVATION_REPORT.md with findings")
print(f"  4. Proceed with historical derivation (1999-2023) for approved features")
